	...
	<div class="row">
		...
		<section class="panel">
			<header class="panel-heading sm" data-color="theme-inverse">
				<h2><strong>{{trans('main.business menu resevables')}}</strong></h2>
			...
			<div class="panel-body">
				<div class="form-group">
					<div>
						<a href="{{url('hotel/business/reservables/create')}}" class="btn btn-theme">{{trans('main.nuevo')}} {{trans('main.resevables')}}</a>
					</div>
				</div>

        {{ Form::open(array('action' => 'HotelReservasController@index', 'files'=>true, 'id'=>'form-service', 'method'=>'GET'))}}
					<div class="form-group">
             ...
             <div>
				...
				<div class="panel panel-default">
			        ...
			        <ul id="sortable" class="col-md-12 list">
			          	...
			          	<?php 
                             $menuLang = ReservablesLang::where('reservables_id', $menu->id)->where('language_id', $lang->language_id)->first();
				  	    ?>
			          	<li id="listItem_{{$menu->id}}" class="col-md-12 item-category" style="padding-bottom: 5px; padding-top: 5px;">
			          		<div class="col-md-2"><img src="{{$menu->picture}}" width="35px" height="35px"></div>
			          		<div class="col-md-5" style="text-align: center;">{{$menuLang->name}}</div>
			          		<div class="col-md-2" >
			          			<div class="row" style="display: block; width: 75px; margin: 0 auto;     height: 25px;">
									<div class="col-sm-4 iSwitch flat-switch">
										<div class="switch">
											<input name="state" class="state" value="{{$menu->id}}" type="checkbox" @if($menu->state==1) checked @endif @if(Reservables::state($menu->id)!=true) disabled @endif>
										</div>
									</div>
								</div>
			          		</div>
			          		<div class="col-md-1">
			          		    <a href="{{url('hotel/business/reservables/'.$menu->id.'/edit')}}" type="button" class="btn btn-md btn-primary"><span class="glyphicon glyphicon-pencil" aria-hidden="true" style="color: white;"></span></a>
			          		</div>
			          		<div class="col-md-1">
			          			<button type="submit" class="btn btn-md btn-warning"  onclick="var notice = new PNotify({text: $('#form_notice_{{$menu->id}}').html(), icon: false, width: 'auto', hide: false, addclass: 'custom', icon: 'picon picon-32 picon-edit-delete', opacity: .8, nonblock: {nonblock: true }, animation: {effect_in: 'show', effect_out: 'slide'}, buttons: {closer: false, sticker: false }, insert_brs: false }); notice.get().find('form.pf-form').on('click', '[name=cancel]', function(){notice.remove(); }).submit(function(){$('#form_notice').submit(); });"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></button>
                                <div id="form_notice_{{$menu->id}}" style="display: none;">
                                  {{ Form::open(array('class'=>'pf-form pform_custom','url' => 'hotel/business/reservables/'.$menu->id)) }}
			          		...
			          		<div class="col-md-1">
			          		     <img src="@if(Reservables::state($menu->id)==true) {{url('assets/img/active.png')}} @else {{url('assets/img/no-active.png')}} @endif" width="35px" height="35px" style="display: block; margin: 0 auto;">
			          		</div>
			          	</li>
			          	@endforeach					
			          </ul>
