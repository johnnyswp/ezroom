...
<div id="welcome" class="col s12 m8 offset-m2">
	<div class="card-panel teal box " style="position:relative">
		...
		<div class="row">
			...
			<a href="{{url('roomer/category/')}}/{{$cat->category_id}}?dt={{ Input::get('dt')}}" class="col s6 m3 waves-effect waves-white">
				<span style="font-size: 1.2rem; display: block; color: white; position: relative; background: rgba(38, 166, 154, 0.82); width: 100%; padding: 0 5px; top: 25px; z-index: 2;" class="card-title truncate"> {{$cat->category_name}} </span>
				<div class="card-image z-depth-2">
					<img src="{{$cat->category_picture}}">
				</div>
			</a>
			@endforeach					 
		</div>	 			 
