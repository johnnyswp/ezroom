	...
	<div class="card-panel teal box " style="position:relative">
		...
		<div class="row">
			...
			<a style="margin: 30px 0;" class="col s12 m12 waves-effect waves-white">
				<span style="font-size: 1.2rem; display: block; color: white; position: relative; background: rgba(38, 166, 154, 0.82); width: 100%; padding: 0 5px; top: 0; z-index: 2;" class="card-title truncate">{{InfoPlaceLang::where('info_place_id',$business->id)->where('language_id',Session::get('lang_id'))->first()->name}}</span>
			...
			<a style="margin-bottom: 30px;"  class="col s12 m12 waves-effect waves-white textP">
				<span>{{InfoPlaceLang::where('info_place_id',$business->id)->where('language_id',Session::get('lang_id'))->first()->description}}</span>
			</a>
			<a href="{{$business->link}}" style="margin-bottom: 30px;"  class="col s12 m12 waves-effect waves-white textP">
				...
				<span style="font-size: 14px;"></>{{InfoPlaceLang::where('info_place_id',$business->id)->where('language_id',Session::get('lang_id'))->first()->link}}</span>
			</a>
