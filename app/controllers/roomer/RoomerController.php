	...
	**/
	...
	public function getLoginLink($link,$lang)
	{
		...
		else
		{
			...
			if($token==$token_){
				...
				Session::put('token_stay', $stay->id);
				$hotel = Hotel::find($stay->hotel_id);
				return Redirect::to('roomer/seleccion')->withHotel($hotel)->withStay($stay);
			}else{
				Session::flash('mgspass', Language::find($lang)->txt_token_invalido);
				#return Redirect::back();
				return View::make('error');
	...
	public function getServicioItem($id)
	{
		...
		$reservables = DB::table('business')
				   	...
				   	->where('reservables.state',1)
				   	->where('reservables.business_id',$service_id)
				   	->groupBy('reservables.id')
				   	->orderBy('reservablesOrder','ASC')
				   	->get();
	...
	public function getReservar($id)
	{
    ...
    public function getItemDetalle($id)
	{
		...
		$business = Menu::where('id',$service_id)->where('hotel_id',$stay->hotel_id)->where('state',1)->first();

		$template = $hotel->theme; 
		return View::make("roomers.themes.$template.item_business")
			->withHotel($hotel)
			 ->withExchange($exchange)
			->withCategories($categories)
			->withPhones($phones)
	...
	public function getCategoriaProductoItem($id)
	{
		...
		return View::make("roomers.themes.$template.item_category_producto")
			->withHotel($hotel)
			 ->withExchange($exchange)
			->withCategories($categories)
			->withPhones($phones)
