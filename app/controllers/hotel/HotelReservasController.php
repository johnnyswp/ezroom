...
class HotelReservasController extends \BaseController {
    ...
    public function edit($id)
    {
        ...
        $item = Reservables::where('id', $id)->where('hotel_id', $hotel->id)->first();
        if($item){
            $weekdays = array('0'=>'Domingo', '1'=>'Lunes', '2'=>'Martes', '3'=>'Miercoles', '4'=>'Jueves', '5'=>'Viernes', '6'=>'Sabado');
            $lang_main = LanguageHotel::where('main', 1)->where('hotel_id', $hotel->id)->first();
            $business = array(''=>trans('main.Seleccione un business'));
            $businessAll = Business::where('hotel_id', $hotel->id)->orderBy('businessOrder', 'ASC');
            foreach($businessAll->get() as $busines){
                $businessLang = BusinessLang::where('business_id', $busines->id)->where('language_id', $lang_main->language_id)->first();
                $business[$busines->id] = $businessLang->name;
            }
            $lang_active = LanguageHotel::where('hotel_id', $hotel->id)->orderBy('main', 'DESC')->orderBy('state', 'DESC');
            return View::make('hotel.pages.reservas.edit_menu_business')->withItem($item)->withHotel($hotel)->withLangs($lang_active->get())->withWeekdays($weekdays)->withBusiness($business);
        }else{
        ...
        if ($validation->fails())
        {
            ...
            if($item->save()){
                ...
                foreach($lang_active->get() as $lang_)
                {
                    ...
                    {
                        $name_item = ReservablesLang::where('language_id', $item->id)->where('language_id', $lang_->language_id)->first();
                        if(!$name_item)
                            $name_item = new ReservablesLang;
                ...
                foreach($langs as $lang)
                {
                    ...
                    {
                        $name_item = ReservablesLang::where('reservables_id', $item->id)->where('language_id', $lang->id)->first();
                        if(!$name_item)
                            $name_item = new ReservablesLang;
                ...
                foreach($weekdays as $weekday => $value)
                {
                    if(Input::get('desde_1_'.$value) or Input::get('hasta_1_'.$value) or Input::get('desde_2_'.$value) or Input::get('hasta_2_'.$value))
                    {
                        $availble = ReservablesAvailable::where('weekday', $weekday)->where('item_id', $item->id)->first();
                        if(!$availble)
                            $availble = new ReservablesAvailable;
                        $availble->item_id = $item->id;
                        $availble->weekday = $weekday;
